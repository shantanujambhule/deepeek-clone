    ...
    ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden '}`}>
      <div>
        <div className={`flex ${expand ? "flex-row gap-10": "flex-col items-center gap-8"}`}>
            ...
            src={expand ? assets.logo_text : assets.logo_icon} alt="logo icon"/>
            <div 
            ...
            h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                <Image src={assets.menu_icon} className='md:hidden' alt='menu icon'/>
                <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} className='md:block hidden w-7'  alt='sidebar icon'/>
                <div className={`absolute w-max ${expand ? "left-1/2 top-12 -translate-x-1/2" : "left-0 -top-12"} opacity-0 group-hover:opacity-100 transition-all duration-300
        ...
        <button onClick={createNewChat} className={`mt-8 flex items-center justify-center cursor-pointer ${
            ...
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2
            rounded-lg shadow-lg pointer-events-none'>New Chat 
        ...
        <div className={`mt-8 text-white/25 text-sm ${expand ? "block" : "hidden"}`}>
            ...
            {
                chats.map((chat,index) => <ChatLabel key={index} name={chat.name} id={chat._id} key={chat._id} openMenu={openMenu} setOpenMenu={setOpenMenu}/>
)}
        </div>
      </div>

      <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? " gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/80 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg" }`}>
            <Image
